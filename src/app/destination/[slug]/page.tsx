...
export default function DestinationPage() {
  ...
  const handleDestinationChange = (newSlug: string) => {
    if (newSlug === "all") {
      router.push("/search");
    } else {
      router.push(`/destination/${newSlug}`);
    }
  };
  ...
  const mapCenter = useMemo(() => {
    const dest = DESTINATION_COORDINATES.find((d) => d.slug === slug);
        ...
        <div className="sticky top-16 z-30 bg-white border-b border-slate-200 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 py-3">
            ...
            <div className="mt-2 text-xs text-slate-500">
              {activities.length} {locale === "en" ? "trips found" : "ทริป"}
            </div>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-6 lg:px-8 py-6">
            {activities.length === 0 ? (
              <div className="text-center py-16">
                <p className="text-slate-500">{t("noActivitiesInCategory")}</p>
                <button
                  type="button"
                  onClick={() => {
