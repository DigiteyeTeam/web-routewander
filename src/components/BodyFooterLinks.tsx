        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {destinations.map((item) => (
                <li key={item.name}>
                  <Link
                    href={`/destination/${item.name.toLowerCase().replace(/\s/g, "-")}`}
                    className="text-slate-600 hover:text-primary text-sm"
                  >
                    {item.name}
                  </Link>
          ...
          <div>
            ...
            <p className="text-slate-600 text-sm">
              <Link href="/" className="hover:text-primary">ไทย</Link>
              <span className="text-slate-400 ml-1">· ทัวร์ & กิจกรรมในประเทศไทย</span>
            </p>
          </div>
          <div>
            <h3 className="font-bold text-slate-800 mb-4 text-sm">
            ...
            <ul className="space-y-2">
              {categories.map((item) => (
                <li key={item.name}>
                  <Link
                    href={`/things-to-do?category=${encodeURIComponent(item.name)}`}
                    className="text-slate-600 hover:text-primary text-sm"
                  >
                    {item.name}
                  </Link>
